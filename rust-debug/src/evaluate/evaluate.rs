...
impl<R: Reader<Offset = usize>> MyPiece<R> {
    ...
    pub fn new(piece: Piece<R>) -> MyPiece<R> {
        MyPiece { piece, used_before: false }
    }
...
impl<R: Reader<Offset = usize>> EvaluatorValue<R> {
    ...
    pub fn evaluate_variable_with_type<M: MemoryAccess>(
        ...
        EvaluatorValue::eval_type(registers, mem, dwarf, &unit, die, data_offset, &mut my_pieces)
    }
            ...
            match pieces[0].piece.clone().location {
                ...
                Location::Address { mut address } => {
                    ...
                    let bytes = match mem.get_address(&(address as u32), num_bytes) {
                        Some(val) => val,
                        None => {
                            error!("can not read address: {:x} num_bytes: {:?}, Return error", address as u64, num_bytes);
                            return Err(anyhow!("can not read address: {:x} num_bytes: {:?}, Return error", address as u64, num_bytes));
                        }
                    };
                ...
                Location::Value { value } => {
                    ...
                    return match parsed_value {
                        ...
                        _ => Ok(EvaluatorValue::Value(parsed_value, ValueInformation {
                            raw: None,
                ...
                Location::Bytes { mut value } => {
                    ...
                    value_pieces.extend_from_slice(&[ValuePiece::Bytes { bytes: bytes.clone() }]);
                ...
                Location::ImplicitPointer { value: _, byte_offset: _ } => {
                    error!("Unimplemented");
            ...
            gimli::DW_TAG_base_type => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_base_type die, this should never happen"));
                    }
                };
            ...
            gimli::DW_TAG_pointer_type => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_pointer_type die, this should never happen"));
                    }
                };
                ...
                let address_class = match attributes::address_class_attribute(die)? {
                    ...
                    None => {
                        ...
                        return Err(anyhow!("Die is missing required attribute DW_AT_address_class"));
                    }
                };
                ...
                let value = match (attributes::type_attribute(dwarf, unit, die)?, &address) {
                    (Some((section_offset, unit_offset)), EvaluatorValue::Value(BaseTypeValue::Address32(address_value), _)) => {
                        ...
                        let header = dwarf.debug_info.header_from_offset(match section_offset.as_debug_info_offset() {
                            ...
                            None => {
                                ...
                                return Err(anyhow!("Could not convert section offset into debug info offset"));
                            }
                        })?;
                        ...
                        let mut new_pieces = vec![MyPiece::new(Piece {
                            size_in_bits: None,
                            bit_offset: None,
                            location: Location::<R>::Address { address: *address_value as u64 },
                        })];
                        EvaluatorValue::eval_type(registers, mem, dwarf, &type_unit, &type_die, 0, &mut new_pieces)?
                    }
                    _ => EvaluatorValue::OptimizedOut,
                };

                Ok(EvaluatorValue::PointerTypeValue(Box::new(PointerTypeValue { name, address, value })))
            ...
            gimli::DW_TAG_array_type => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_array_type die, this should never happen"));
                    }
                };
                ...
                let subrange_type_value = match EvaluatorValue::eval_type(registers, mem, dwarf, unit, &dimension_die, data_offset, pieces)? {
                ...
                match subrange_type_value.get_count()? {
                    Some(count) => {
                        ...
                        // Evaluate all the values in the array.
                        for _i in 0..count {
                            values.push(EvaluatorValue::eval_type(registers, mem, dwarf, &type_unit, type_die, data_offset, pieces)?);
                        }
                    }
                    None => (),
                };

                Ok(EvaluatorValue::Array(Box::new(ArrayTypeValue { subrange_type_value, values })))
            }
            gimli::DW_TAG_structure_type => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_structure_type die, this should never happen"));
                    }
                };
                ...
                let name = match attributes::name_attribute(dwarf, die)? {
                    ...
                    None => {
                        ...
                        return Err(anyhow!("Expected the structure type die to have a name attribute"));
                    }
                };
                ...
                for c in &children {
                    ...
                    match c_die.tag() {
                        ...
                        gimli::DW_TAG_variant_part => {
                            // Get the value.
                            let members = vec![EvaluatorValue::eval_type(registers, mem, dwarf, unit, &c_die, data_offset, pieces)?];

                            return Ok(EvaluatorValue::Struct(Box::new(StructureTypeValue { name, members })));
                        }
                        gimli::DW_TAG_member => {
                ...
                for member_die in &member_dies {
                    let member = match member_die.1.tag() {
                        gimli::DW_TAG_member => EvaluatorValue::eval_type(registers, mem, dwarf, unit, &member_die.1, data_offset, pieces)?,
                        tag => {
                ...
                Ok(EvaluatorValue::Struct(Box::new(StructureTypeValue { name, members })))
            }
            gimli::DW_TAG_union_type => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_union_type die, this should never happen"));
                    }
                };
                ...
                Ok(EvaluatorValue::Union(Box::new(UnionTypeValue { name, members })))
            }
            gimli::DW_TAG_member => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_member die, this should never happen"));
                    }
                };
                ...
                // Evaluate the value.
                let value = EvaluatorValue::eval_type(registers, mem, dwarf, &type_unit, type_die, new_data_offset, pieces)?;

                Ok(EvaluatorValue::Member(Box::new(MemberValue { name, value })))
            }
            gimli::DW_TAG_enumeration_type => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_enumeration_type die, this should never happen"));
                    }
                };
                ...
                // Get type value.
                let variant = EvaluatorValue::eval_type(registers, mem, dwarf, &type_unit, type_die, data_offset, pieces)?;
                ...
                let name = match attributes::name_attribute(dwarf, die)? {
                    ...
                    None => {
                        ...
                        return Err(anyhow!("Expected enumeration type die to have attribute DW_AT_name"));
                    }
                };

                Ok(EvaluatorValue::Enum(Box::new(EnumerationTypeValue { name, variant, enumerators })))
            }
            gimli::DW_TAG_variant_part => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_variant_part die, this should never happen"));
                    }
                };
                ...
                let variant: Option<MemberValue<R>> = match attributes::discr_attribute(die)? {
                    Some(die_offset) => {
                        ...
                        match member_die.tag() {
                            gimli::DW_TAG_member => match EvaluatorValue::eval_type(registers, mem, dwarf, unit, member_die, data_offset, pieces)? {
                ...
                for c in &children {
                    ...
                    if c_die.tag() == gimli::DW_TAG_variant {
                        ...
                        let variant = match EvaluatorValue::eval_type(registers, mem, dwarf, unit, &c_die, data_offset, &mut temp_pieces)? {
                        ...
                        if let (Some(discr_value), Some(variant_num)) = (variant.discr_value, variant_number) {
                ...
                Ok(EvaluatorValue::VariantPartValue(Box::new(VariantPartValue { variant, variants })))
            }
            gimli::DW_TAG_variant => {
                ...
                for c in children {
                    ...
                    if c_die.tag() == gimli::DW_TAG_member {
                        ...
                        let member = match EvaluatorValue::eval_type(registers, mem, dwarf, unit, &c_die, data_offset, pieces)? {
            ...
            gimli::DW_TAG_subrange_type => {
                ...
                match die.tag() {
                    ...
                    _ => {
                        ...
                        return Err(anyhow!("Expected DW_TAG_subrange_type die, this should never happen"));
                    }
                };
                    ...
                    None => {
                        ...
                        let (type_unit, die_offset) = match get_type_info(dwarf, unit, die) {
                            ...
                            Err(_) => {
                                ...
                                return Err(anyhow!("Expected subrange type die to have type information"));
                            }
                        };
                        let type_die = &type_unit.entry(die_offset)?;

                        // Evaluate the type attribute value.
                        let base_type_value = match EvaluatorValue::eval_type(registers, mem, dwarf, &type_unit, type_die, data_offset, pieces)? {
                            EvaluatorValue::Value(base_type_value, value_information) => Some((base_type_value, value_information)),
                            _ => {
                                error!("Unreachable");
                                return Err(anyhow!("Unreachable"));
                            }
...
fn get_type_info<R: Reader<Offset = usize>>(
    ...
    let (unit_offset, die_offset) = match attributes::type_attribute(dwarf, unit, die)? {
        ...
        None => {
            ...
            return Err(anyhow!("Die doesn't have the required DW_AT_type attribute"));
        }
    };
    let unit = match unit_offset {
        gimli::UnitSectionOffset::DebugInfoOffset(offset) => {
...
fn check_alignment<R: Reader<Offset = usize>>(die: &gimli::DebuggingInformationEntry<'_, '_, R>, mut data_offset: u64, pieces: &Vec<MyPiece<R>>) -> Result<()> {
...
fn get_children<R: Reader<Offset = usize>>(unit: &gimli::Unit<R>, die: &gimli::DebuggingInformationEntry<'_, '_, R>) -> Result<Vec<gimli::UnitOffset>> {
