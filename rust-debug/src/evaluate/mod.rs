    ...
    if let Ok(Some(tattr)) = die.attr_value(gimli::DW_AT_type) {
        match tattr {
            gimli::AttributeValue::UnitRef(offset) => {
                ...
                return evaluate(dwarf, unit, pc, expr, frame_base, Some(unit), Some(&die), registers, mem);
            }
            gimli::AttributeValue::DebugInfoRef(di_offset) => {
                ...
                while let Ok(Some(header)) = iter.next() {
                    ...
                    if let Some(offset) = offset.to_unit_offset(&type_unit) {
                        ...
                        return evaluate(dwarf, unit, pc, expr, frame_base, Some(&type_unit), Some(&die), registers, mem);
                    }
                }
...
pub fn evaluate_value<R: Reader<Offset = usize>, T: MemoryAccess>(
    ...
    match type_unit {
        Some(unit) => match type_die {
            Some(die) => {
                info!("with type info");
                return EvaluatorValue::evaluate_variable_with_type(dwarf, registers, mem, &pieces, unit.header.offset(), die.offset());
            }
            None => (),
        },
        None => (),
...
pub fn evaluate_pieces<R: Reader<Offset = usize>, T: MemoryAccess>(
    ...
    loop {
        match result {
            ...
            RequiresRegister { register, base_type } => match registers.get_register_value(&register.0) {
                Some(data) => {
            ...
            RequiresCallFrameCfa => {
                result = eval.resume_with_call_frame_cfa(registers.cfa.ok_or_else(|| anyhow!("Requires CFA"))? as u64)?;
            }

            RequiresAtLocation(die_ref) => match die_ref {
                DieReference::UnitRef(unit_offset) => help_at_location(dwarf, unit, pc, &mut eval, &mut result, frame_base, unit_offset, registers, mem)?,
                ...
                DieReference::DebugInfoRef(debug_info_offset) => {
                    ...
                    if let Some(unit_offset) = debug_info_offset.to_unit_offset(&unit_header) {
                        ...
                        help_at_location(dwarf, &new_unit, pc, &mut eval, &mut result, frame_base, unit_offset, registers, mem)?;
                    } else {
            ...
            RequiresEntryValue(entry) => {
                let entry_value = evaluate(dwarf, unit, pc, entry, frame_base, None, None, registers, mem)?;

                result = eval.resume_with_entry_value(convert_to_gimli_value(match entry_value.to_value() {
                    Some(val) => val,
                    None => {
                        error!("Optimized Out");
            ...
            RequiresParameterRef(unit_offset) => {
                ...
                let value = evaluate(dwarf, unit, pc, expr, frame_base, Some(unit), Some(&die), registers, mem)?;
            ...
            RequiresIndexedAddress { index: _, relocate: _ } => {
...
/// `base_type`.
fn eval_base_type<R>(unit: &gimli::Unit<R>, data: Vec<u8>, base_type: gimli::UnitOffset<usize>) -> Result<BaseTypeValue>
where
    R: Reader<Offset = usize>,
{
    ...
    let encoding = match die.attr_value(gimli::DW_AT_encoding)? {
        ...
        _ => {
            ...
            return Err(anyhow!("Expected base type die to have attribute DW_AT_encoding"));
        }
    };
