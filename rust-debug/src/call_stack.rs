...
use gimli::{
    AttributeValue::{DebugInfoRef, DebugStrRef, UnitRef},
    DebugFrame,
...
use crate::{
    evaluate::evaluate::{BaseTypeValue, EvaluatorValue, ValueInformation},
    registers::Registers,
    source_information::SourceInformation,
    utils::{die_in_range, get_current_unit, DwarfOffset},
    variable::{is_variable_die, Variable},
};
...
pub trait MemoryAccess {
    ...
    ///
    /// This function is used for reading `num_bytes` bytes in the debugged
    /// target system at the address `address`.
    /// This is done when evaluating variables that are stored in the memory of
    /// the debugged target.
...
/// * `registers` - A `Registers` struct which is used to read the register
///   values.
...
/// Then it will evaluate all the variables in each of the stack frames, and
/// return a `Vec` of `StackFrame`s.
pub fn stack_trace<'a, R: Reader<Offset = usize>, M: MemoryAccess>(
    ...
    //    println!("\n\nnew stack trace");
    //    let das = new_stack_trace(dwarf, debug_frame, registers.clone(), memory,
    // cwd)?;    for sf in &das {
...
pub fn new_stack_trace<'a, R: Reader<Offset = usize>, M: MemoryAccess>(
    ...
    let pc_reg = registers.program_counter_register.ok_or_else(|| anyhow!("Requires pc register id"))?;
    let link_reg = registers.link_register.ok_or_else(|| anyhow!("Requires pc register id"))?;
    let sp_reg = registers.stack_pointer_register.ok_or_else(|| anyhow!("Requires pc register id"))?;
    ...
    new_stack_trace_rec(
        ...
        &mut Box::new(gimli::UnwindContext::<R::Offset>::new()),
    )
}
...
pub fn new_stack_trace_rec<'a, R: Reader<Offset = usize>, M: MemoryAccess>(
    ...
    ctx: &mut gimli::UnwindContext<R::Offset>,
) -> Result<Vec<StackFrame<R>>> {
    // Check current pc.
    ...
    // Get CFA
    let cfa = unwind_cfa::<R>(unwind_registers, unwind_info)?;
                    ...
                    let value = match memory.get_address(&address, 4) {
                        Some(val) => {
                            ...
                            u32::from_le_bytes(match result.as_slice().try_into() {
                                Ok(val) => val,
                                Err(err) => {
                                    error!("When creating u32 in offset {:?}", err);
                        ...
                        None => {
                            ...
                            return Err(anyhow!("Could not read 4 bytes from address 0x{:x}", address));
                        }
                    };
                    value
                };
    ...
    stack_trace.push(create_stack_frame(dwarf, call_frame, registers, memory, cwd)?);
    ...
    let header = dwarf.debug_info.header_from_offset(match section_offset.as_debug_info_offset() {
        Some(val) => val,
        None => return Err(anyhow!("Could not convert section offset to debug info offset")),
    })?;
    ...
    let next_code_location = match die.attr_value(gimli::DW_AT_inline)? {
        Some(val) => {
            error!("Unexpected inlined function with attribute value: {:?}", val);
            return Err(anyhow!("Unexpected inlined function with attribute value: {:?}", val));
        }
        None => {
            if die.tag() == gimli::DW_TAG_inlined_subroutine {
                ...
                // Call address is equal to return address, but the first bit needs to be
                // removed because of thumb mode.
...
/// * `debug_frame` - A reference to the DWARF section `.debug_frame`.
///
/// This function will virtually unwind the call stack and return a `Vec` of
/// `CallFrame`s.
pub fn unwind_call_stack<R: Reader<Offset = usize>, M: MemoryAccess>(registers: Registers, memory: &mut M, debug_frame: &'_ DebugFrame<R>) -> Result<Vec<CallFrame>> {
    let pc_reg = registers.program_counter_register.ok_or_else(|| anyhow!("Require pc register id"))?;
    let link_reg = registers.link_register.ok_or_else(|| anyhow!("Requires pc register id"))?;
    let sp_reg = registers.stack_pointer_register.ok_or_else(|| anyhow!("Requires pc register id"))?;
                ...
                let value = {
                    let value = match memory.get_address(&address, 4) {
                        ...
                        None => {
                            ...
                            return Err(anyhow!("Can not read 4 bytes from address 0x{:x}", address));
                        }
                    };
                    value
                };
...
fn unwind_cfa<R: Reader<Offset = usize>>(registers: [Option<u32>; 16], unwind_info: &gimli::UnwindTableRow<R::Offset>) -> Result<Option<u32>> {
...
impl<R: Reader<Offset = usize>> StackFrame<R> {
    ...
    ///
    /// This function will go through each of the variables in this stack frame
    /// and return the one with the same name as the given name.
...
pub fn create_stack_frame<M: MemoryAccess, R: Reader<Offset = usize>>(
    ...
    let fb_header = dwarf.debug_info.header_from_offset(match fb_section_offset.as_debug_info_offset() {
        Some(val) => val,
        None => return Err(anyhow!("Could not convert section offset to debug info offset")),
    })?;
    ...
    let frame_base = match evaluate_frame_base(dwarf, &unit, pc, &fb_die, &mut temporary_registers, mem) {
        Ok(val) => val,
        Err(err) => {
            error!("While retrieving frame base {:?}", err);
            return Err(anyhow!("{:?}", err));
        }
    };
    ...
    for variable_die in dies_to_check {
        let vc = match Variable::get_variable(
            ...
            Ok(v) => v,
            Err(err) => {
                log::error!("While getting variable Error: {:?}", err);
                continue;
            }
        };
...
///
/// * `dwarf` - A reference to gimli-rs `Dwarf` struct.
/// * `address` - Used to find which function this machine code address belongs
///   too.
///
/// This function will search DWARF for the function that the given machine code
/// address belongs too.
pub fn find_function_die<R: Reader<Offset = usize>>(dwarf: &'_ Dwarf<R>, address: u32) -> Result<(gimli::UnitSectionOffset, gimli::UnitOffset)> {
...
/// This function will search DWARF for the function that the given machine code
/// address belongs too.
pub fn find_non_inlined_function_die<R: Reader<Offset = usize>>(dwarf: &'_ Dwarf<R>, address: u32) -> Result<(gimli::UnitSectionOffset, gimli::UnitOffset)> {
    let unit = get_current_unit(dwarf, address)?;
    let mut cursor = unit.entries();
...
///
/// * `dwarf` - A reference to gimli-rs `Dwarf` struct.
/// * `section_offset` - A offset into the DWARF `.debug_info` section which is
///   used to find the
/// relevant compilation.
/// * `unit_offset` - A offset into the given compilation unit, which points the
///   subroutine DIE.
/// * `pc` - A machine code address, it is usually the current code address.
///
/// This function will go done the subtree of a subroutine DIE and return all in
/// range variable DIEs.
pub fn get_functions_variables_die_offset<R: Reader<Offset = usize>>(
    ...
    fn recursive_offset<R: Reader<Offset = usize>>(dwarf: &Dwarf<R>, unit: &Unit<R>, node: EntriesTreeNode<R>, pc: u32, list: &mut Vec<UnitOffset>) -> Result<()> {
        let die = node.entry();
...
/// * `memory` - Used to read the memory of the debugged target.
///
/// This function is used to evaluate the frame base address for a given
/// subroutine.
...
fn is_argument<R: Reader<Offset = usize>>(dwarf: &Dwarf<R>, section_offset: UnitSectionOffset, unit_offset: UnitOffset) -> Result<bool> {
