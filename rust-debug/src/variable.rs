...
impl<R: Reader<Offset = usize>> Variable<R> {
    ...
    pub fn get_variable<M: MemoryAccess>(
        ...
        let pc: u32 = *registers
            .get_register_value(
                &(registers
                    ...
                    .ok_or_else(|| anyhow!("Requires that the program counter register is known"))? as u16),
            )
            .ok_or_else(|| anyhow!("Requires that the program counter registers has a value"))?;

        // Get the variable die.
        let header = dwarf.debug_info.header_from_offset(match dwarf_offset.section_offset.as_debug_info_offset() {
            Some(val) => val,
            None => {
        ...
        let value = evaluate(dwarf, &unit, pc, expression, frame_base, Some(&type_unit), Some(&type_die), registers, memory)?;

        Ok(Variable { name, value, source })
    }
}
...
pub fn is_variable_die<R: Reader<Offset = usize>>(die: &DebuggingInformationEntry<R>) -> bool {
    ...
    die.tag() == gimli::DW_TAG_variable || die.tag() == gimli::DW_TAG_formal_parameter || die.tag() == gimli::DW_TAG_constant
}
...
pub fn get_var_name<R: Reader<Offset = usize>>(dwarf: &Dwarf<R>, unit: &Unit<R>, die: &DebuggingInformationEntry<R>) -> Result<Option<String>> {
...
pub fn find_variable_location<R: Reader<Offset = usize>>(dwarf: &Dwarf<R>, unit: &Unit<R>, die: &DebuggingInformationEntry<R>, address: u32) -> Result<VariableLocation<R>> {
    if is_variable_die(die) {
...
pub fn find_variable_type_die<R: Reader<Offset = usize>>(dwarf: &Dwarf<R>, unit: &Unit<R>, die: &DebuggingInformationEntry<R>) -> Result<(UnitSectionOffset, UnitOffset)> {
    if is_variable_die(die) {
