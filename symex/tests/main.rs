...
mod llvm_tests {
    ...
    //
    // This also has backtracking so it ensures it correctly handles re-entry into
    // an inner function.
    #[test]
    fn call() {
    ...
    //         let res = run("tests/samples/array_index.bc",
    // "array_index::get_unchecked");         assert_eq!(res.len(), 1, "expected
    // 1 path");         assert_eq!(
    //             res[0],
